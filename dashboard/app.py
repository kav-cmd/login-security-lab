...
def load_defense_config():
    """Load current defense configuration"""
    return {
...
def main():
    ...
    with st.sidebar:
        ...
        for defense, enabled in defenses.items():
            status = "🟢 ON" if enabled else "🔴 OFF"
            st.markdown(f"**{defense}**: {status}")
        ...
        st.markdown("---")
        st.info("💡 To change defenses, edit `config.py` and restart the Flask app")
